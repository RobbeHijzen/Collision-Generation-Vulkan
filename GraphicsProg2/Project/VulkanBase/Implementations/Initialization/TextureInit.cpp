        ...
        if (shader->SupportsImages())
        {
            for (const auto& meshIndex : shader->GetReferencedMeshIndices())
            {
                ...
                if (auto mesh3D{ dynamic_cast<Mesh3D*>(mesh) })
                {
                    ...
                    stbi_uc* pixels = stbi_load(mesh3D->GetDiffuseString().c_str(), &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
                    ...
                    CreateImage(texWidth, texHeight, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL, VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
                        VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, m_TextureImages[mesh3D->GetTextureIndex()], m_TextureImagesMemory[mesh3D->GetTextureIndex()]);


                    TransitionImageLayout(m_TextureImages[mesh3D->GetTextureIndex()], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
                    CopyBufferToImage(stagingBuffer, m_TextureImages[mesh3D->GetTextureIndex()], static_cast<uint32_t>(texWidth), static_cast<uint32_t>(texHeight));
                    TransitionImageLayout(m_TextureImages[mesh3D->GetTextureIndex()], VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL);
